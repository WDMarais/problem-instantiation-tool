...
def srs_instance():
    ...
    registry = InMemoryRegistry({"quadratic_factor": problem})
...
def two_step_ca_instance():
    ...
    registry = InMemoryRegistry({"two_step_linear": problem})
    engine = Engine(registry=registry)
    return engine.instantiate("two_step_linear", params={"a": 5})
...
def mcq_instance():
    ...
    registry = InMemoryRegistry({"mcq_geo": problem})
    engine = Engine(registry=registry)
    return engine.instantiate("mcq_geo", params={"correct": "B"})
...
def self_graded_instance():
    ...
    registry = InMemoryRegistry({"worked_example_check": problem})
    engine = Engine(registry=registry)
    return engine.instantiate("worked_example_check", seed=1)
...
def gap_fill_instance():
    ...
    engine = Engine(registry=registry)
    return engine.instantiate("surd_gap", params={"a": 3})
...
def ca_gap_noncontiguous_instance():
    ...
    engine = Engine(registry=registry)
    return engine.instantiate("three_step_ca_gap", params={"a": 5})
...
def high_marks_instance():
    ...
    engine = Engine(registry=registry)
    return engine.instantiate("circle_insight", params={"radius": 1})
...
def test_partial_attempt_marks_possible_covers_submitted_only(two_step_ca_instance):
    ...
    attempt = SolutionAttempt(steps=[SubmittedStep(5)])
    rating = two_step_ca_instance.verifier.rate(attempt)
    assert rating.marks_possible == 1
    assert rating.marks_awarded == 1
...
def test_gap_fill_provided_steps_skipped_in_marking(gap_fill_instance):
    ...
    s2_value = gap_fill_instance.presented_attempt.steps[2].value
    ...
    rating = gap_fill_instance.verifier.rate(attempt)
...
def test_provided_step_breaks_ca_chain_in_noncontiguous_gap_fill(
    ...
    rating = ca_gap_noncontiguous_instance.verifier.rate(attempt)
...
def test_none_step_in_rate_raises_attempt_validation_error(srs_instance):
    ...
    attempt = SolutionAttempt(steps=[None])
    with pytest.raises(AttemptValidationError) as exc_info:
        srs_instance.verifier.rate(attempt)
...
def test_high_marks_possible_awarded_in_full_on_correct(high_marks_instance):
    ...
    canonical_value = high_marks_instance.solution.steps[0].value
    attempt = SolutionAttempt(steps=[SubmittedStep(canonical_value)])
    rating = high_marks_instance.verifier.rate(attempt)
...
def test_mcq_correct_choice(mcq_instance):
    ...
    attempt = SolutionAttempt(steps=[SubmittedStep("B")])
    rating = mcq_instance.verifier.rate(attempt)
    assert rating.steps[0].mistake_type == "correct"
...
def test_mcq_wrong_choice(mcq_instance):
    ...
    attempt = SolutionAttempt(steps=[SubmittedStep("A")])
    rating = mcq_instance.verifier.rate(attempt)
    assert rating.steps[0].mistake_type == "computation_error"
    assert rating.steps[0].marks_awarded == 0
    assert rating.is_correct is False
...
def test_self_graded_true_is_correct(self_graded_instance):
    ...
    attempt = SolutionAttempt(steps=[SubmittedStep(True)])
    rating = self_graded_instance.verifier.rate(attempt)
    assert rating.steps[0].mistake_type == "correct"
...
def test_self_graded_false_is_not_correct(self_graded_instance):
    ...
    attempt = SolutionAttempt(steps=[SubmittedStep(False)])
    rating = self_graded_instance.verifier.rate(attempt)
    assert rating.steps[0].marks_awarded == 0
    assert rating.is_correct is False
